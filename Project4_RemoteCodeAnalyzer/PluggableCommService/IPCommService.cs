 ...
 * ver 1.0 : 15 Jun 2017
 * - first release
 * ver 1.1 : 25 Aug 2018 Mudit Vats
 * - added access, batch, file commands
 */
using System;
...
namespace PluggableRepository
{
    ...
    public class CommMessage
    {
        ...
        public enum Command
        {
            ...
            login,                // login command
            [EnumMember]
            announceBatch,        // announce new batch (to server)
            [EnumMember]
            execAnalytics,        // run analytics (on the server)
            [EnumMember]
            getAnalyticsFileList, // run analytics (on the server)
            [EnumMember]
            getAnalytics,         // run analytics (on the server)
            [EnumMember]
            announceFile,         // announce file
            [EnumMember]
            getBatchList,         // get batch list
            [EnumMember]
            getBatchFileList,     // get files list contained in a batch
            [EnumMember]
            downloadFiles,        // download files
            [EnumMember]
            getUsersList,         // get users
            [EnumMember]
            greatFileAccess,      // grant access
            [EnumMember]
            addComment,           // grant access
            [EnumMember]
            getComments,           // grant access
            [EnumMember]
            searchBatch ,         // grant access
            [EnumMember]
            searchFiles          // grant access
        }
